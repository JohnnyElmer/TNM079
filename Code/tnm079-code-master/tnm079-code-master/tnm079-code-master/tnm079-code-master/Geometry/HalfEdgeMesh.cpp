...
bool HalfEdgeMesh::AddFace(const std::vector<glm::vec3>& verts) {
    ...
    // Connect inner ring
    e(pair1.first).next = pair2.first;
    e(pair1.first).prev = pair3.first;

    e(pair2.first).next = pair3.first;
    e(pair2.first).prev = pair1.first;

    e(pair3.first).next = pair1.first;
    e(pair3.first).prev = pair2.first;

    // Finally, create the face, don't forget to set the normal (which should be
    // normalized)
    Face tri;
    tri.edge = pair1.first;
    ...
    e(pair1.first).face = faceIndex;
    e(pair2.first).face = faceIndex;
    e(pair3.first).face = faceIndex;
...
std::vector<size_t> HalfEdgeMesh::FindNeighborVertices(size_t vertexIndex) const {
    ...
    // Find next edge to save outer vertex then go to prev edge and find pair to itr to next
    // triangle and save next edge vert.
...
std::vector<size_t> HalfEdgeMesh::FindNeighborFaces(size_t vertexIndex) const {
    ...
    // Find vertexIndex edge and face, save face and itr to prev edge and save their faces until we
    // found starting edge again.
    size_t startIndex = v(vertexIndex).edge;
    size_t index = startIndex;
...
float HalfEdgeMesh::VertexCurvature(size_t vertexIndex) const {
    ...
    //Arean varierar beroende på vilken oneRing vi kollar på, angleSum är ungefär samma för alla oneRings.
...
glm::vec3 HalfEdgeMesh::VertexNormal(size_t vertexIndex) const {
    ...
    for (int i = 0; i < neighborFaces.size(); i++) {
        n += FaceNormal(neighborFaces.at(i));
    }
...
float HalfEdgeMesh::Area() const {
    ...
    for (int i = 0; i < mFaces.size(); i++) {
        const HalfEdgeMesh::HalfEdge E0 = e(mFaces.at(i).edge);
        const HalfEdgeMesh::HalfEdge E1 = e(E0.next);
        const HalfEdgeMesh::HalfEdge E2 = e(E0.prev);

        auto v1 = v(E0.vert).pos;
        auto v2 = v(E1.vert).pos;
        auto v3 = v(E2.vert).pos;

        area += glm::length(glm::cross(v2 - v1, v3 - v1)) * 0.5f;
    }
...
float HalfEdgeMesh::Volume() const {
    ...
    for (int i = 0; i < mFaces.size(); i++) {
        ...
        area = glm::length(glm::cross(v2 - v1, v3 - v1)) * 0.5f;
        volume += (glm::dot(((v1 + v2 + v3) / glm::vec3(3.f, 3.f, 3.f)), mFaces.at(i).normal * area) / 3.f);
    }

...
glm::vec3 LoopSubdivisionMesh::VertexRule(size_t vertexIndex) {
    ...
    std::vector<size_t> neighbourVert = FindNeighborVertices(vertexIndex);
    
    //valens=antalet bindningar
    float k = neighbourVert.size();

    //Get the current vertex
    glm::vec3 vtx = v(vertexIndex).pos * (1 - k * Beta(k));

    //Look at the weights to correctly place the new vertex
    for (int i = 0; i < neighbourVert.size(); i++) {
        vtx += v(neighbourVert[i]).pos * Beta(k);
    }
...
glm::vec3 LoopSubdivisionMesh::EdgeRule(size_t edgeIndex) {
    ...
    return (((v0 + v1) * (3.f / 8.f)) + ((v2 + v3) * (1.f / 8.f)));
}
